...
def get_module_path(version: str) -> Path:
    """Locate the bison module path."""
    relative_repo_root = Path(__file__).parent.parent.parent
    repo_root = Path(os.getenv("BUILD_WORKSPACE_DIRECTORY", str(relative_repo_root)))

    return repo_root / f"modules/bison/{version}"
...
def collect_template_values(
    ...
    inline_pattern = re.compile(
        r"^\s+-e\s+'\/([\w\s_]+)\/r \$\(([\w_]+)\)'", re.MULTILINE
    )
...
def main() -> None:
    ...
    # config_h = module_path / f"config_{args.platform}.h"
