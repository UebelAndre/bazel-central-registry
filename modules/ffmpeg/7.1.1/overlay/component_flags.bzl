...
_ALL_COMPONENTS = PROFILE_EVERYTHING
...
def component_select_srcs(exclusive_map, shared_list):
    ...
    groups_by_key = {
        tuple(comps): gid
        for gid, comps in SHARED_GROUP_DEFINITIONS
    }
...
_DEFAULTS = {c: True for c in CONFIGURE_DEFAULT_COMPONENTS}
_MACOS = {c: True for c in MACOS_COMPONENTS}
_LINUX = {c: True for c in LINUX_COMPONENTS}
...
def _with_defaults_impl(_settings, attr):
    ...
    if os == "macos":
        ...
        platform = {}
    return {
        str(Label("//:enable_{}".format(comp))): comp in _DEFAULTS or comp in platform
