...
def resolve_have_list(
    raw_vars: dict[str, str], cache: dict[str, list[str]]
) -> list[str]:
...
def generate_config_h_in(
    ...
    lines.append("")
    lines.append(
        "/* --- CONFIG_EXTRA subsystems (resolved by component selection) --- */"
    )
    ...
    lines.append("")
    lines.append(
        "// FFmpeg requires all detection macros defined to 0 or 1 (not undefined)."
    )
    lines.append(
        "// Provide defaults for any that were not set by autoconf checks above."
    )
