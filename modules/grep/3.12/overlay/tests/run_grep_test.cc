...
//
// Naming convention follows grep_smoke_test.cc: snake_case for functions and
// variables, PascalCase for types, so nothing collides with Win32 macros.
...
// configure probes these with gt_LOCALE_FR / gt_LOCALE_FR_UTF8, but locale
...
// `fr_FR`. "none" is what init.cfg treats as "no such locale".
#ifdef _WIN32
constexpr const char* kLocaleFrCandidates[] = {"French_France.1252"};
constexpr const char* kLocaleFrUtf8Candidates[] = {"French_France.65001"};
#else
constexpr const char* kLocaleFrCandidates[] = {"fr_FR", "fr_FR.ISO-8859-1",
                                               "fr_FR.ISO8859-1"};
constexpr const char* kLocaleFrUtf8Candidates[] = {"fr_FR.UTF-8", "fr_FR.utf8",
                                                   "fr.UTF-8"};
#endif

template <std::size_t N>
std::string first_working_locale(const char* const (&candidates)[N]) {
...
// The value of `#define NAME "value"` in config.h. `help-version` checks
// `grep --version` against $VERSION and several tests print
// $PACKAGE_BUGREPORT, so both come from the header grep was built with.
std::string config_define(const fs::path& config_h, std::string_view name) {
  ...
  die(std::string(name) + " is not defined as a string in " +
      config_h.string());
}

void write_file(const fs::path& path, std::string_view content) {
  std::ofstream out(path, std::ios::binary | std::ios::trunc);
  if (!out) die("cannot create " + path.string());
...
std::string test_shell() {
  ...
  for (std::size_t begin = 0; begin <= path.size();) {
    ...
    if (end > begin) {
      ...
      const std::string lowered = [&dir] {
        std::string s = generic(dir);
...
// Quote one token for the platform's command interpreter: single quotes for
// /bin/sh, doubled double quotes for cmd.exe -- the same rules as
// grep_smoke_test.cc.
...
#ifdef _WIN32
  // cmd.exe /c strips the outermost quote pair when the line both begins
  // and ends with `"`, which shell_quote guarantees; wrap once more so the
  // strip is harmless. See grep_smoke_test.cc.
  cmd = "\"" + cmd + "\"";
...
int main(int argc, char** argv) {
  ...
  const std::string tmp_dir = env_or_empty("TEST_TMPDIR");
  if (tmp_dir.empty()) {
    die("TEST_TMPDIR is not set; not running under `bazel test`?");
  }
  const fs::path stage = fs::path(tmp_dir) / "tree";
  ...
  // The binaries keep their own file names so `grep.exe` stays `grep.exe`.
  // get-mb-cur-max is upstream's check_PROGRAMS helper (without it on PATH
  // every multibyte test skips); it is declared in this package, so where
  // Bazel materializes a runfiles tree the copy above already brought it
  // over -- read-only, like every Bazel output -- and copy_file cannot
  // overwrite that in place. Remove first, then copy.
  const auto stage_binary = [&ec](const fs::path& from, const fs::path& to) {
    fs::remove(to, ec);
  ...
  set_env("LOCALE_FR", first_working_locale(kLocaleFrCandidates));
  set_env("LOCALE_FR_UTF8", first_working_locale(kLocaleFrUtf8Candidates));
  set_env("PACKAGE_BUGREPORT", config_define(config_h, "PACKAGE_BUGREPORT"));
  set_env("VERSION", config_define(config_h, "VERSION"));
  set_env("AWK", "awk");
