...
def _parse_version_date(version):
    """Parse YYYY.MM or YYYY.M from the start of version, return (year, month) or None."""
    parts = version.split(".")
    if len(parts) < 2:
        return None
    year_s = parts[0]
...
def _generate_timestamp_h_impl(ctx):
    ...
    parsed = _parse_version_date(ctx.attr.version)
    if parsed:
        year, month = parsed
    else:
        year, month = _FALLBACK_YEAR, _FALLBACK_MONTH

    month_abbr, month_num = _MONTHS[month]
    year_s = str(year)

    ctx.actions.write(
        output = out,
        content = """\
#define U_BOOT_DATE "{month_abbr} 01 {year}"
#define U_BOOT_TIME "00:00:00"
#define U_BOOT_TZ "+0000"
#define U_BOOT_DMI_DATE "{month_num}/01/{year}"
#define U_BOOT_BUILD_DATE 0x{year}{month_num}01
""".format(
...
generate_timestamp_h = rule(
    ...
    attrs = {
        ...
        "version": attr.string(
            doc = "The current u-boot version.",
            mandatory = True,
        ),
    },
)
